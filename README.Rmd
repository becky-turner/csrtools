...
This package is particularly suited for researchers in medicine, and psychology, or any field that involves meta-analytical studies and time-series data.
...
```{r}
bctq_data <- data(bctq_data)

```

## Example: Run standardised mean change meta-analysis

```{r smc1, warning=FALSE, fig.height=9, fig.width=12}
...
Note the `open_smc_data` includes only data for the Open CTR subset of patients. The dataset was curated using the `smc_tp` function above to calculate the SMC between each time-point.
...
smc_lines <- smc_smooth %>% filter(parameter == "smc")
